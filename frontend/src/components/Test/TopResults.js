...
const TopResults = () => {
  ...
  const topResults = async () => {
    ...
    try {
      await fetch(
        "https://projekt-fer.herokuapp.com/web//test/liveResults?classId=" +
      ...
      <div>
        <nav>
          <div className="header">
            <div className="titleTopResults title-all pointer title-design">
              <img
                width="40"
                height="40"
                src={"../images/space-invaders.png"}
              />
              <p onClick={() => goHome()}>Top rezultati</p>
            </div>
          ...
          return (
            <div
              ...
              className={
                ...
                colors[Math.floor(Math.random() * 21)]
              }
            >
              <span>{i + 1}</span>
              <span>{item.name}</span>
